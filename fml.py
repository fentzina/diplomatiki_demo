...
def get_case_id(path):
    """
    Extracts case_id from the file path.
    Example: '100029_00001_0000.nii.gz' -> '100029_00001'
    """
...
def process_case(ct_path, label_path, output_dir):
    ...
    # =========================================================================
    # 1. RUN COLIAGE ON THE FULL ISOTROPIC VOLUMES
    # =========================================================================
    logger.info("Executing 3D CoLIAGe on full volume...")
    
    # CoLIAGe expects input in (Height, Width, Depth) -> (Y, X, Z)
    full_ct_yxz   = np.transpose(clipped_ct_numpy, (1, 2, 0))
    full_mask_yxz = np.transpose(guide_mask, (1, 2, 0))
    
    # Initialize and execute on full volume (let Collage handle boundaries internally)
    SVD_RADIUS = 3
    ...
    # Run original calculation method cleanly
    full_collage_output = collage_3d_instance.execute()
    logger.info(f"Full volume CoLIAGe output shape: {full_collage_output.shape}")

    # =========================================================================
    # 2. TRANSPOSE AND EXTRACT OUR PRECISS 128^3 CROP
    # =========================================================================
    # Transpose back from YXZ to ZYX: (Y, X, Z, 13, 2) -> (Z, Y, X, 13, 2)
    full_haralick_volume_zyx = np.transpose(full_collage_output, (2, 0, 1, 3, 4))
    
    # Extract our exact target crop matching our standard coordinates
    _3d_haralick_volume_zyx = full_haralick_volume_zyx[z_start:z_end, y_start:y_end, x_start:x_end]
    ...
    # Flatten 13 features × 2 angles -> 26 channels: (128, 128, 128, 26)
