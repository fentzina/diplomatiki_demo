# -*- coding: utf-8 -*-
"""step6_Attention-weighted_CNN_training.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1t9QAt30PgCRQZYYtNxO9Z3O0l4OANxwn
"""
...
OUTPUT_DIR       = "/home/student1/ftzina_thesis/step6/step6_attention_weighted_outputs"
...
# ── Attention-weighted combination ────────────────────────────────────────────
# A small attention network that learns which of the 4 patch embeddings
# is most informative for each patient. Produces one weighted patient-level
# embedding (128-dim) rather than a scalar probability.
# Reference: Ilse et al. (2018) "Attention-based Deep Multiple Instance Learning"

class AttentionAggregator(nn.Module):
    """
    Learns a scalar attention weight per patch embedding.
    Input : (N_patches, embed_dim)
    Output: (embed_dim,) — weighted sum of patch embeddings
    """
    def __init__(self, embed_dim: int = 128, hidden_dim: int = 64):
        ...
        self.attention = nn.Sequential(
            ...
            nn.Tanh(),
            nn.Linear(hidden_dim, 1)          # one scalar score per patch
        )

    def forward(self, patch_embeddings: torch.Tensor) -> torch.Tensor:
        # patch_embeddings: (N_patches, embed_dim)
        scores  = self.attention(patch_embeddings)          # (N_patches, 1)
        weights = torch.softmax(scores, dim=0)              # (N_patches, 1) — sum to 1
        patient_emb = (weights * patch_embeddings).sum(0)  # (embed_dim,)
        return patient_emb, weights.squeeze(1)             # also return weights for inspection


# Instantiate ONCE after loading the best CNN checkpoint
attention_net = AttentionAggregator(embed_dim=EMBED_DIM, hidden_dim=64).to(device)

# NOTE: the attention network is NOT trained here — it is initialized with
# random weights and used as a fixed aggregation mechanism.
# To train it jointly, you would need to retrain the full model from scratch
# with the attention network included in the forward pass.
# For now we use it as a post-hoc aggregator, which is a valid approach
# (Ilse et al. also use a pre-trained feature extractor with a separately
# trained attention network in their implementation).
attention_net.eval()

def extract_train_embeddings_attention(case_ids_ordered: np.ndarray) -> np.ndarray:
    """
    Training split: extract 4 patch embeddings per case, pass them through
    the attention network to produce one weighted patient-level embedding (128-dim).

    Output shape: (N, 128) — one embedding per patient.
    """
    ...
    emb_accumulator = {}   # case_id → list of (128,) embedding arrays

    with torch.no_grad():
        for X_batch, _, case_ids_batch in train_emb_loader:
            # return_embedding=True → get 128-dim vectors, not probabilities
    ...
    # Apply attention aggregation per patient
    patient_embeddings = []
    for cid in case_ids_ordered:
        patch_embs = np.stack(emb_accumulator[str(cid)], axis=0)  # (4, 128)
        patch_tensor = torch.tensor(patch_embs, dtype=torch.float32).to(device)
        with torch.no_grad():
            patient_emb, attn_weights = attention_net(patch_tensor)  # (128,), (4,)
        patient_embeddings.append(patient_emb.cpu().numpy())

    emb_matrix = np.stack(patient_embeddings, axis=0)  # (N, 128)
    print(f"  train: {emb_matrix.shape}  "
          f"(attention-aggregated {PATCHES_PER_CASE} patches × {len(case_ids_ordered)} cases)")
    return emb_matrix.astype(np.float32)

def extract_val_test_embeddings_attention(case_ids_ordered: np.ndarray,
                                          split_name: str) -> np.ndarray:
    """
    Val / test splits: 4 seeded random patches per case → attention aggregation
    → one 128-dim patient embedding. Symmetric with training procedure.
    ...
    with torch.no_grad():
        for case_id in case_ids_ordered:
            ...
            # 4 seeded random patches — deterministic per case
            ...
            for center in centers:
                ...
                # return_embedding=True → 128-dim vector
                emb = model(x, return_embedding=True)        # (1, 128)
                patch_embs.append(emb.squeeze(0))            # (128,)

            # Stack and apply attention
            patch_tensor = torch.stack(patch_embs, dim=0)   # (4, 128)
            patient_emb, attn_weights = attention_net(patch_tensor)  # (128,), (4,)
            patient_embeddings.append(patient_emb.cpu().numpy())

    emb_matrix = np.stack(patient_embeddings, axis=0)       # (N, 128)
    print(f"  {split_name}: {emb_matrix.shape}  "
          f"({PATCHES_PER_CASE} seeded patches, attention-aggregated)")
    return emb_matrix.astype(np.float32)

# ── Run extraction ─────────────────────────────────────────────────────────────
print("── Extracting patient-level embeddings (attention-weighted) ──")
print("   Train : 4 random patches → attention aggregation → (N, 128)")
print("   Val   : 4 seeded patches → attention aggregation → (N, 128)")
print("   Test  : 4 seeded patches → attention aggregation → (N, 128)\n")

model.load_state_dict(
    torch.load(os.path.join(OUTPUT_DIR, "best_cnn.pt"), map_location=device)
)
model.eval()

deep_train = extract_train_embeddings_attention(id_train)
deep_val   = extract_val_test_embeddings_attention(id_val,  "val")
deep_test  = extract_val_test_embeddings_attention(id_test, "test")

np.save(os.path.join(OUTPUT_DIR, "deep_embeddings_train_attention.npy"), deep_train)
np.save(os.path.join(OUTPUT_DIR, "deep_embeddings_val_attention.npy"),   deep_val)
np.save(os.path.join(OUTPUT_DIR, "deep_embeddings_test_attention.npy"),  deep_test)

print(f"\nSaved attention-aggregated embeddings:")
print(f"  deep_embeddings_train_attention.npy → {deep_train.shape}")
print(f"  deep_embeddings_val_attention.npy   → {deep_val.shape}")
print(f"  deep_embeddings_test_attention.npy  → {deep_test.shape}")
print("\nStep 6 (attention) complete. Use *_attention.npy files in step7_fusion.py.")
